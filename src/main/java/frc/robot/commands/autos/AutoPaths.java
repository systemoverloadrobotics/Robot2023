...
public class AutoPaths {
    public static PPSwerveControllerCommand getPathCommand(Swerve swerve, PathName path) {
        PathPlannerTrajectory trajectory = PathPlanner.loadPath(path.toString(), Constants.Swerve.SWERVE_MAX_AUTO_SPEED,
                Constants.Swerve.SWERVE_MAX_ACCELERATION);
        PPSwerveControllerCommand command = new PPSwerveControllerCommand(trajectory, swerve::getOdometryPose,
                Constants.RobotDimensions.SWERVE_DRIVE_KINEMATICS, Constants.Auto.X_PID_CONTROLLER,
                Constants.Auto.Y_PID_CONTROLLER, Constants.Auto.ROT_PID_CONTROLLER,
                (SwerveModuleState[] states) -> swerve.setModuleStates(states), false, swerve);
        return command;
    }
