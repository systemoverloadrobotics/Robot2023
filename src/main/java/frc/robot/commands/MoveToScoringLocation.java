...
public class MoveToScoringLocation extends CommandBase {
    ...
    public void initialize() {
        ...
        if (intelligentScoring.getGridOffset(scoringLocation) == GridOffset.LEFT) {
            scoringLocationPose =
                    new Pose2d(currentPose.getX() + offsetLeft, currentPose.getY(), currentPose.getRotation());
        } else if (intelligentScoring.getGridOffset(scoringLocation) == GridOffset.RIGHT) {
            scoringLocationPose =
                    new Pose2d(currentPose.getX() + offsetRight, currentPose.getY(), currentPose.getRotation());
        } else {
            scoringLocationPose = currentPose;
        }
        futureTrajectory = AsyncTrajectory.generateTrajectory(currentPose, scoringLocationPose, new ArrayList<>(),
