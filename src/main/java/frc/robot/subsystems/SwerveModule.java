...
public class SwerveModule extends SubsystemBase {
  ...
  public SwerveModule(String name, int powerID, int steerID, double offset) {
    ...
    powerControllerConfig.setCurrentLimit(
      Constants.Swerve.SWERVE_POWER_CURRENT_LIMIT
    );
    powerControllerConfig.setMaxOutput(
      Constants.Swerve.SWERVE_POWER_MAX_OUTPUT
    );
    ...
    powerController =
      new SuSparkMax(
        new CANSparkMax(powerID, MotorType.kBrushless),
        name + " Power",
    ...
    steerControllerConfig.setCurrentLimit(
      Constants.Swerve.SWERVE_ROTATION_CURRENT_LIMIT
    );
    steerControllerConfig.setMaxOutput(
      Constants.Swerve.SWERVE_ROTATION_MAX_OUTPUT
    );

    SensorConfiguration steerSensorConfig = new SensorConfiguration(
      new SensorConfiguration.ConnectedSensorSource(
        4096,
        1,
        ConnectedSensorType.PWM_ENCODER
      )
    );

    steeringController =
      new SuSparkMax(
        new CANSparkMax(steerID, MotorType.kBrushless),
        name + " Steer",
        steerControllerConfig,
        steerSensorConfig
      );

    ((CANSparkMax) steeringController.rawController()).getAbsoluteEncoder(
  ...
  public SwerveModuleState getState() {
    return new SwerveModuleState(
      this.powerController.outputVelocity(),
